...
router.get("/:id", async function (req, res) {
  try {
    ...
    if (employee != null) res.send(employee);
    else res.status(404).send({ error: true, message: "not found." });
  } catch (error) {
    res.status(400).send({ error: true, message: "bad input." });
  }
});
...
router.delete("/", async function (req, res) {
  try {
    ...
    if (numOfRows != null) res.send({ success: true, message: "deleted." });
    else res.status(404).send({ error: true, message: "not found." });
  } catch (error) {
    res.status(400).send({ error: true, message: "bad input." });
  }
...
router.delete("/:id", async function (req, res) {
  try {
    ...
    if (numOfRows != null) res.send({ id: req.params.id });
    else res.status(404).send({ error: true, message: "not found." });
  } catch (error) {
    res.status(400).send({ error: true, message: "bad input." });
  }
  ...
  try {
    ...
    var parser = parse({ columns: true }, async function (err, data) {
      ...
      if (employees.length > 0) {
        ...
        res
          .status(500)
          .send({ error: true, message: "Database related internal error." });
      }
    });
